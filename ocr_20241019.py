...
model_dir = 'drive/My Drive/PaymentReceipt/model/cnn_b5.h5'
...
transno_pattern = re.compile(r"^(Transaction No|Transaction ID|Transaction IO|Invoice No)\s?:?\s?(.+)")
...
def extract_date_time(date_time_str):
    ...
    date_pattern = re.compile(r"(\d{8})|(\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|\d{1,2} \w+ \d{4}|\w+ \d{1,2}, \d{4})")
...
def extract_transaction_data(text):
    ...
    for line in lines:
        ...
        # Amount (if Amount Field does not exist.)
             ...
             amount_only_extracted = amount_only_pattern_match.group(1).replace("-","").strip()
...
for filename in os.listdir(image_dir):
    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
        ...
        try:
            ...
            transaction_info["Payment Type"] = predit_class(image_path, class_labels) # Image Classification
            # 2024/10/19 HNDW End
